ShortestPath <- function(Costs, Source, Destination) {
  # Given a matrix of costs this function determines the shortest path between a
  ...
  while (TRUE) {
    ...
    cost <- min(TempLabels) # Determines label of lower label node
    ...
    for (k in seq_along(TempNodes)) { # Update labels of temporary nodes
